...
def compute_bin_medians(wedge_vals, bin_indices, n_bins):
    ...
    for bin_i in range(n_bins):
        count = 0
        any_nan = False  # Flag to mark if a NaN is encountered in this bin
        # First pass: count values in this bin and check for NaNs
        for j in range(wedge_vals.shape[0]):
            if bin_indices[j] == bin_i:
                if np.isnan(wedge_vals[j]):
                    any_nan = True
                count += 1
        # If no values or if any value is NaN, mark the bin as NaN
        if count == 0 or any_nan:
            result[bin_i] = np.nan
        else:
            # Otherwise, collect the values and compute the median
...
def compute_wedge_radial_profiles(image, mask, base_center, center,
                                  dx_base, dy_base,
                                  n_wedges=8, n_rad_bins=200, r_min=0, r_max=None):
    """
    Compute radial profiles for image wedges using a shifted center.

    This function computes wedge profiles by first shifting the precomputed coordinate 
    differences (dx_base, dy_base) based on the effective center relative to a fixed base 
    center. The shift is calculated as:
        shift = (center[0] - base_center[0], center[1] - base_center[1])
    The new coordinate differences are then:
        new_dx = dx_base - shift[0]   (equivalent to: columns - center[0])
        new_dy = dy_base - shift[1]   (equivalent to: rows - center[1])
    From these, the radial distances and angles are computed. The function bins the 
    image values in both radial and angular directions to generate wedge-specific 
    radial profiles.
    ...
    Parameters:
      ...
      n_wedges : int, optional
          Number of angular wedges to divide the image into (default is 8).
      n_rad_bins : int, optional
          Number of radial bins for computing the profiles in each wedge (default is 200).
      r_min : float, optional
          Minimum radial distance from the center to start the binning (default is 0).
